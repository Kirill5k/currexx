...
/** A pool of reusable TestServices instances to avoid the overhead of creating new services for each evaluation during genetic algorithm
  * optimization.
  *
  * @param available
  *   Queue of available TestServices ready to be acquired
  * @param poolSize
...
final class TestServicesPool[F[_]] private (
  ...
  /** Acquire a TestServices instance from the pool, reset it with new settings, and return it ready for use.
    *
...
object TestServicesPool {
  ...
  /** Create a new pool of TestServices instances.
    ...
    *   Settings to initialize the services with
    * @param poolSize
    *   Number of services to maintain in the pool (should match parallelism level)
    */
