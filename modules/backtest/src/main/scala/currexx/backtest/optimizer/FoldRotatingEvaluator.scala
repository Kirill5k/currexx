...
/** Scores an indicator from its per-fold backtests, withholding one search fold per generation.
  ...
  *
  * Cheap because the withholding is in the aggregation and not in the backtest: `backtests` is expected to be memoised and to return every
...
final class FoldRotatingEvaluator[F[_]: Functor](
    backtests: Indicator => F[List[List[OrderStats]]],
  ...
  override def evaluateIndividual(indicator: Indicator, phase: EvaluationPhase): F[(Indicator, Fitness)] =
    backtests(indicator).map { foldStats =>
      val scores = foldStats.map(scoringFunction.score)
