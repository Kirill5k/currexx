...
object TestStrategy {
  ...
  // median win-to-loss ratio: 2.595375, total profit: 0.13789, total orders: 1986, median profit: 0.01783, median loss: -0.0038730731225296443
  val s1_optimized = TestStrategy(
    indicator = Indicator.compositeAnyOf(
      Indicator.TrendChangeDetection(
        source = ValueSource.HLC3,
        transformation = ValueTransformation.NMA(length = 100, signalLength = 40, lambda = 0.8, maCalc = currexx.domain.signal.MovingAverage.Exponential)
      ),
      Indicator.ThresholdCrossing(
        source = ValueSource.Close,
        transformation = ValueTransformation.STOCH(length = 85),
        upperBoundary = 90.0,
        lowerBoundary = 11.0
      ),
      Indicator.ValueTracking(
  ...
  val s2_v2_optimized = TestStrategy(
    indicator = Indicator.compositeAnyOf(
      Indicator.LinesCrossing(
        source = ValueSource.HLC3,
