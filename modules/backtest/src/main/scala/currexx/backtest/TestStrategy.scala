...
object TestStrategy {
  ...
  // ga-optimisation-2026-07-29-1606-s1_optimized_shuffle.md (fitness 1.494212, shuffled GA).
  // net=6130.03186, closed=1502, forced=5, win=73.04%, exp=4.081246, PF=1.383, DD=1.47%, Sharpe=4.217, gross=7600.72902, costs=1470.69715
  val s1_optimized_v2 = TestStrategy(
    indicator = Indicator.compositeAnyOf(
      Indicator.TrendChangeDetection(
        source = ValueSource.HLC3,
        transformation = ValueTransformation.NMA(length = 7, signalLength = 14, lambda = 8.0, maCalc = MovingAverage.Exponential)
      ),
      Indicator.ThresholdCrossing(
        source = ValueSource.Close,
        transformation = ValueTransformation.STOCH(length = 31),
        upperBoundary = 90.0,
        lowerBoundary = 26.0
      ),
      // ATR (absolute price units) feeds the price-distance stop via lastVolatilityValue.
      Indicator.ValueTracking(
        role = ValueRole.Volatility,
        source = ValueSource.Close,
        transformation = ValueTransformation.ATR(length = 22)
      ),
      // Close price feeds the stop via lastClosePrice.
      Indicator.ValueTracking(
        role = ValueRole.Price,
        source = ValueSource.Close,
        transformation = ValueTransformation.SMA(length = 46)
      )
    ),
    rules = TradeStrategy(
      openRules = List(
  ...
  // GA-optimized indicator params for s1_optimized (rules unchanged). Champion from
  // ga-optimisation-2026-07-29-1416-s1_optimized.md (fitness 1.430565). The 1545 run of the same
  // round converged on an identical champion, so it is promoted only once.
  ...
  // GA-optimized indicator params for s1_optimized (rules unchanged). Champion from
  // ga-optimisation-2026-07-29-1437-s1_optimized_shuffle.md (fitness 1.215020, shuffled GA).
  // net=3829.74961, closed=1467, forced=5, win=70.62%, exp=2.610600, PF=1.275, DD=1.16%, Sharpe=2.723, gross=5265.10300, costs=1435.35339
  val s1_optimized_v4 = TestStrategy(
    indicator = Indicator.compositeAnyOf(
      Indicator.TrendChangeDetection(
        source = ValueSource.HLC3,
        transformation = ValueTransformation.NMA(length = 9, signalLength = 14, lambda = 4.5, maCalc = MovingAverage.Exponential)
      ),
      Indicator.ThresholdCrossing(
        source = ValueSource.Close,
        transformation = ValueTransformation.STOCH(length = 34),
        upperBoundary = 77.0,
        lowerBoundary = 39.0
      ),
      // ATR (absolute price units) feeds the price-distance stop via lastVolatilityValue.
      Indicator.ValueTracking(
        role = ValueRole.Volatility,
        source = ValueSource.Close,
        transformation = ValueTransformation.ATR(length = 21)
      ),
      // Close price feeds the stop via lastClosePrice.
      Indicator.ValueTracking(
        role = ValueRole.Price,
        source = ValueSource.Close,
        transformation = ValueTransformation.SMA(length = 94)
      )
    ),
    rules = TradeStrategy(
      openRules = List(
  ...
  // ga-optimisation-2026-07-29-1919-s1_v2_optimized_v2.md (fitness 2.242213).
  // net=10331.60730, closed=207, forced=6, win=61.35%, exp=49.911146, PF=3.885, DD=0.72%, Sharpe=5.913, gross=10538.31077, costs=206.70347
  val s1_v2_optimized_v3 = TestStrategy(
    indicator = Indicator.compositeAnyOf(
      // Primary signal: JMA crossover
      Indicator.LinesCrossing(
        source = ValueSource.HLC3,
        line1Transformation = ValueTransformation.JMA(length = 39, phase = -57, power = 4),
        line2Transformation = ValueTransformation.JMA(length = 19, phase = 8, power = 8)
      ),
      // Momentum filter
      Indicator.ThresholdCrossing(
        source = ValueSource.Close,
        transformation = ValueTransformation.RSX(length = 34),
        upperBoundary = 73.0,
        lowerBoundary = 17.0
      ),
      // Momentum tracking
      ...
      Indicator.VolatilityRegimeDetection(
        atrLength = 24,
        smoothingType = ValueTransformation.SMA(length = 59)
      )
    ),
    rules = TradeStrategy(
      openRules = List(
  ...
  // ga-optimisation-2026-07-29-1817-s1_v2_shuffle.md (fitness 2.175846, shuffled GA).
  // net=9839.28779, closed=637, forced=5, win=50.86%, exp=15.446292, PF=2.073, DD=0.53%, Sharpe=4.099, gross=10457.84714, costs=618.55934
  val s1_v2_optimized_v4 = TestStrategy(
    indicator = Indicator.compositeAnyOf(
      // Primary signal: JMA crossover
      Indicator.LinesCrossing(
        source = ValueSource.HLC3,
        line1Transformation = ValueTransformation.JMA(length = 22, phase = -18, power = 3),
        line2Transformation = ValueTransformation.JMA(length = 37, phase = -15, power = 3)
      ),
      // Momentum filter
      Indicator.ThresholdCrossing(
        source = ValueSource.Close,
  ...
  // GA-optimized indicator params for s1_v2_optimized_v2 (rules unchanged). Champion from
  // ga-optimisation-2026-07-29-2018-s1_v2_optimized_v2_shuffle.md (fitness 2.161040, shuffled GA).
  // net=10439.14019, closed=548, forced=6, win=48.54%, exp=19.049526, PF=2.292, DD=0.64%, Sharpe=5.579, gross=10967.11959, costs=527.97940
  val s1_v2_optimized_v5 = TestStrategy(
    indicator = Indicator.compositeAnyOf(
      // Primary signal: JMA crossover
      Indicator.LinesCrossing(
        source = ValueSource.HLC3,
        line1Transformation = ValueTransformation.JMA(length = 21, phase = -65, power = 3),
        line2Transformation = ValueTransformation.JMA(length = 38, phase = -31, power = 3)
      ),
      // Momentum filter
      Indicator.ThresholdCrossing(
        source = ValueSource.Close,
        transformation = ValueTransformation.RSX(length = 36),
        upperBoundary = 69.0,
        lowerBoundary = 12.0
      ),
      // Momentum tracking
  ...
  // GA-optimized indicator params for s1_v2 (rules unchanged). Champion from
  // ga-optimisation-2026-07-29-1715-s1_v2.md (fitness 1.823914).
  // net=3107.22793, closed=172, forced=1, win=73.84%, exp=18.065279, PF=3.563, DD=0.34%, Sharpe=5.655, gross=3276.91883, costs=169.69091
  val s1_v2_optimized_v6 = TestStrategy(
    indicator = Indicator.compositeAnyOf(
      // Primary signal: JMA crossover
      Indicator.LinesCrossing(
        source = ValueSource.HLC3,
        line1Transformation = ValueTransformation.JMA(length = 43, phase = -83, power = 2),
        line2Transformation = ValueTransformation.JMA(length = 13, phase = -20, power = 6)
      ),
      // Momentum filter
      Indicator.ThresholdCrossing(
        source = ValueSource.Close,
  ...
  // ga-optimisation-2026-07-29-2119-s2.md (fitness 1.602777).
  // net=4324.86587, closed=558, forced=2, win=78.67%, exp=7.750656, PF=1.902, DD=0.51%, Sharpe=4.024, gross=4868.56845, costs=543.70258
  val s2_optimized_v3 = TestStrategy(
    indicator = Indicator.compositeAnyOf(
      Indicator.LinesCrossing(
        source = ValueSource.HLC3,
        line1Transformation = ValueTransformation.JMA(length = 43, phase = -90, power = 1),
        line2Transformation = ValueTransformation.JMA(length = 24, phase = 93, power = 6)
      ),
      Indicator.ThresholdCrossing(
        source = ValueSource.Close,
        transformation = ValueTransformation.RSX(length = 16),
