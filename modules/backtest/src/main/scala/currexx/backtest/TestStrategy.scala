...
object TestStrategy {
  ...
  // median win-to-loss ratio: 3.3947, total profit: 0.41657, total orders: 512, median profit: 0.06847, median loss: -0.004178
  val s4_optimized = TestStrategy(
    indicator = Indicator.compositeAnyOf(
      Indicator.TrendChangeDetection(
        source = ValueSource.HLC3,
        transformation = ValueTransformation.JMA(length = 50, phase = -76, power = 1)
      ),
      Indicator.KeltnerChannel(
        source = ValueSource.Close,
  ...
  // median win-to-loss ratio: 0.8711, total profit: 0.22505, total orders: 281, median profit: 0.03764, median loss: -0.001783
  val s8_optimized = TestStrategy(
    indicator = Indicator.compositeAnyOf(
      Indicator.KeltnerChannel(
        source = ValueSource.Close,
        middleBand = ValueTransformation.EMA(length = 42),
        atrLength = 12,
        atrMultiplier = 0.6
      ),
      Indicator.ValueTracking(
        ...
        source = ValueSource.HLC3,
        transformation = ValueTransformation.KalmanVelocity(gain = 0.28, measurementNoise = 0.01)
      ),
      Indicator.VolatilityRegimeDetection(
        atrLength = 23,
      ...
      Indicator.PriceLineCrossing(
        source = ValueSource.Close,
        role = ValueRole.Momentum,
        transformation = ValueTransformation.ParabolicSAR(afStart = 0.025, afMax = 0.14, afStep = 0.015)
      )
    ),
    rules = TradeStrategy(
      openRules = List(
  ...
  // median win-to-loss ratio: 1.6401, total profit: 0.23207, total orders: 251, median profit: 0.04492, median loss: -0.001437
  ...
  // median win-to-loss ratio: 0.9155, total profit: 0.18439, total orders: 1738, median profit: 0.03209, median loss: -0.000683
  val s10_optimized = TestStrategy(
    indicator = Indicator.compositeAnyOf(
      Indicator.TrendChangeDetection(
        source = ValueSource.HLC3,
        transformation = ValueTransformation.JMA(length = 25, phase = 49, power = 9)
      ),
      Indicator.ThresholdCrossing(
        source = ValueSource.Close,
      ...
      Indicator.ValueTracking(
        role = ValueRole.Velocity,
        source = ValueSource.HLC3,
        transformation = ValueTransformation.KalmanVelocity(gain = 0.31, measurementNoise = 0.01)
      )
    ),
    rules = TradeStrategy(
      openRules = List(
