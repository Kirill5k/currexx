...
//          Rule.Condition.volatilityIsHigh
  ...
  val s4_indicators = Indicator.compositeAnyOf(
    Indicator.BollingerBands(
      source = ValueSource.Close,
      middleBand = ValueTransformation.SMA(20),
      stdDevLength = 20,
    ...
    Indicator.ValueTracking(
      ...
      source = ValueSource.Close,
      transformation = ValueTransformation.SMA(20)
    )
  )

  val s4_strategy = TradeStrategy(
    openRules = List(
      Rule(
        action = TradeAction.OpenShort,
        conditions = Rule.Condition.allOf(
    ...
    closeRules = List(
      Rule(
        action = TradeAction.ClosePosition,
        conditions = Rule.Condition.anyOf(
          // --- TAKE-PROFIT Condition: Price reverts to the mean ---
          // For a short position, exit when price falls back to the middle band.
          Rule.Condition.allOf(
            Rule.Condition.positionIsSell,
            Rule.Condition.PriceCrossedLine(ValueRole.ChannelMiddleBand, Direction.Downward)
          ),
          // For a long position, exit when price rallies back to the middle band.
          Rule.Condition.allOf(
            Rule.Condition.positionIsBuy,
            Rule.Condition.PriceCrossedLine(ValueRole.ChannelMiddleBand, Direction.Upward)
          ),

          // --- STOP-LOSS Condition: The fade failed and a trend is forming against us ---
          // If we are short and a strong uptrend forms, get out immediately.
          Rule.Condition.allOf(
            Rule.Condition.positionIsSell,
            Rule.Condition.TrendIs(Direction.Upward)
          ),
          // If we are long and a strong downtrend forms, get out immediately.
          Rule.Condition.allOf(
            Rule.Condition.positionIsBuy,
