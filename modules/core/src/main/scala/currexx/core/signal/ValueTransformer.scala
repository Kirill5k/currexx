  ...
  extension (vt: VT)
    private def transform(data: List[Double], ref: MarketTimeSeriesData): List[Double] =
      vt match
        ...
        case VT.ATR(length)                            => Volatility.averageTrueRange(data, ref.highs, ref.lows, length)
        ...
        case VT.ADX(length)                            => MomentumOscillators.averageDirectionalIndex(ref.closings, ref.highs, ref.lows, length)
        case VT.WilliamsR(length)                      => MomentumOscillators.williamsR(ref.closings, ref.highs, ref.lows, length)
        case VT.CCI(length)                            => MomentumOscillators.commodityChannelIndex(ref.closings, ref.highs, ref.lows, length)
        case VT.IchimokuKijunSen(length)               => MomentumOscillators.ichimokuKijunSen(ref.highs, ref.lows, length)
        case VT.ParabolicSAR(afStart, afMax, afStep)   => MomentumOscillators.parabolicSAR(ref.highs, ref.lows, afStart, afMax, afStep)
        case VT.CMF(length)                            => MomentumOscillators.chaikinMoneyFlow(ref.closings, ref.highs, ref.lows, ref.volumes, length)
