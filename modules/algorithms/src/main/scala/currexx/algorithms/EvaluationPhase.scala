...
enum EvaluationPhase:
  /** One of the search's own scorings, numbered from zero for the initial population.
    *
    * An objective is free to read a different slice of its evidence per generation - withholding a fold in rotation, say, so that selection
    * cannot climb one regime - which is what makes these figures incomparable between generations and across runs.
    */
  case Search(generation: Int)

  /** The finished population, scored on everything the objective has.
    *
    * Run once, after selection has stopped and before validation, so that the training figure a run reports means the same as another run's
    * however either of them sliced its evidence along the way.
    */
  case Rescore
