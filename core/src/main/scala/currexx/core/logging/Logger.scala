...
final private class LiveLogger[F[_]](
  ...
  override def error(t: Throwable)(message: => String): F[Unit] =
    enqueue(LogLevel.Error, t, message) *> logger.error(t)(message)

  override def error(message: => String): F[Unit] =
    enqueue(LogLevel.Error, message) *> logger.error(message)

  override def warn(t: Throwable)(message: => String): F[Unit] =
    enqueue(LogLevel.Warn, t, message) *> logger.warn(t)(message)

  override def info(t: Throwable)(message: => String): F[Unit] =
    enqueue(LogLevel.Info, t, message) *> logger.info(t)(message)

  override def debug(t: Throwable)(message: => String): F[Unit] =
    enqueue(LogLevel.Debug, t, message) *> logger.debug(t)(message)

  override def trace(t: Throwable)(message: => String): F[Unit] =
    enqueue(LogLevel.Trace, t, message) *> logger.trace(t)(message)

  override def warn(message: => String): F[Unit] =
    enqueue(LogLevel.Warn, message) *> logger.warn(message)

  override def info(message: => String): F[Unit] =
    enqueue(LogLevel.Info, message) *> logger.info(message)

  override def debug(message: => String): F[Unit] =
    enqueue(LogLevel.Debug, message) *> logger.debug(message)

  override def trace(message: => String): F[Unit] =
    enqueue(LogLevel.Trace, message) *> logger.trace(message)
}
