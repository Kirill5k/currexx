...
trait TradeSettingsRepository[F[_]] extends Repository[F]:
  def get(uid: UserId): F[TradeParameters]
...
final private class LiveTradeSettingsRepository[F[_]](
  ...
  override def get(uid: UserId): F[TradeParameters] =
    collection
      .find(userIdEq(uid))
      .first
      .flatMap(gs => F.fromOption(gs, AppError.NotSetup("Global")))
      .flatMap(gs => F.fromOption(gs.getAs[TradeParameters]("trade"), AppError.NotSetup("Trade")))

object TradeSettingsRepository:
  def make[F[_]: Async](db: MongoDatabase[F]): F[TradeSettingsRepository[F]] =
    db.getCollectionWithCodec[Document]("settings").map(LiveTradeSettingsRepository[F](_))
